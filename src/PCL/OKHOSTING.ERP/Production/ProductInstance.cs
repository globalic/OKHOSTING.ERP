...
namespace OKHOSTING.ERP.New.Production
{
	...
	public class ProductInstance
	{
		...
		/// or a unique identifier like a domain name or a username
		/// </remarks>
		public void InvoiceItem_AfterInsert(InvoiceItem item)
		{
			//if ProductInstanceType is null, exit
			...
			using (var db = Core.BaitAndSwitch.Create<DataBase>())
			{
				...
				select.Where.Add(new ORM.Filters.ValueCompareFilter() { Member = dtype[dm => dm.Name], ValueToCompare = item.Description });
				select.Where.Add(new ORM.Filters.ValueCompareFilter() { Member = productType[dm => dm.ProductInstanceType], ValueToCompare = item.Product.ProductInstanceType, TypeAlias = select.Joins.First().Alias });
