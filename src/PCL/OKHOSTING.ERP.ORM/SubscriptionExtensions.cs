...
namespace OKHOSTING.ERP.ORM
{
	public static class SubscriptionExtensions
	{
		...
		public static bool IsFirstItem(this Subscription subscription, InvoiceItem item)
		{
			...
			return subscription.Items.IndexOf(item) == 0;
		}
		...
		public static bool IsLastItem(this Subscription subscription, InvoiceItem item)
		{
			...
			return subscription.Items.IndexOf(item) == subscription.Items.Count - 1;
		}
		...
		public static bool IsUpgrade(this Subscription subscription, InvoiceItem item)
		{
			...
			SubscriptionProduct spOld = subscription.Items[subscription.Items.IndexOf(item) - 1].Product as SubscriptionProduct;
			double days = item.Invoice.Date.Subtract(subscription.Items[subscription.Items.IndexOf(item) - 1].Invoice.Date).TotalDays;
